...
#   @micropython.native
  ...
  SMALL_FONT = {
      ...
      'b': [0xFE, 0x12, 0x22, 0x22, 0x1C],
      'c': [0x1C, 0x22, 0x22, 0x22, 0x04],
      'd': [0x1C, 0x22, 0x22, 0x12, 0xFE],
      'e': [0x1C, 0x22, 0x3E, 0x22, 0x1C],
      'f': [0x1C, 0x2A, 0x2A, 0x2A, 0x18],
      'g': [0x10, 0x2A, 0x2A, 0x2A, 0x3C],
      'h': [0xFE, 0x10, 0x20, 0x20, 0x1E],
      'i': [0x00, 0x22, 0xBE, 0x02, 0x00],
      'j': [0x04, 0x02, 0x22, 0xBC, 0x00],
      'k': [0xFE, 0x08, 0x14, 0x22, 0x00],
      'l': [0x00, 0x82, 0xFE, 0x02, 0x00],
      'm': [0x3E, 0x20, 0x18, 0x20, 0x1E],
      'n': [0x3E, 0x10, 0x20, 0x20, 0x1E],
      'o': [0x1C, 0x22, 0x22, 0x22, 0x1C],
      'p': [0x3E, 0x28, 0x28, 0x28, 0x10],
      'q': [0x10, 0x28, 0x28, 0x18, 0x3E],
      'r': [0x3E, 0x10, 0x20, 0x20, 0x10],
      's': [0x12, 0x2A, 0x2A, 0x2A, 0x04],
      ...
      'y': [0x30, 0x0A, 0x0A, 0x0A, 0x3C],
      'z': [0x22, 0x26, 0x2A, 0x32, 0x22],
  }

  def draw_small_text(self, pos, text, color, mirror=False):
      """Draw small 5x8 text at pos (x,y). mirror=True flips horizontally (fixes backwards/right-to-left text)."""
      x, y = pos
      current_x = x
      for char in text:
          if char in self.SMALL_FONT:
              bitmap = self.SMALL_FONT[char]
              for row in range(8):
                  for col in range(5):
                      bit_col = col if not mirror else (4 - col)  # Reverse bit order for mirror
                      if bitmap[bit_col] & (1 << (7 - row)):
                          pixel_x = current_x + col  # ALWAYS use original col order for x position
                          self.pixel((pixel_x, y + row), color)
              current_x += 6  # 5px wide + 1px spacing
          else:
              current_x += 6  # Space for unknown chars
